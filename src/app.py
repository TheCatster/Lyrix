...
class Lyrix:
    def __init__(self, master):
        ...
        root.title(get_title(self.scope, self.username, self.client_id, self.client_secret, self.redirect_uri))
        self.text = tk.Text(root, wrap='word', font=('PT Sans', '18'))
        self.scroll_y = tk.Scrollbar(root, orient='vertical', command=self.text.yview)

        get_lyrics(self.scope, self.username, self.client_id, self.client_secret, self.redirect_uri,
                   self.client_access_token)
        self.name = get_name(self.scope, self.username, self.client_id, self.client_secret, self.redirect_uri)
